...
func (m model) View() string {
	...
	// ── ヘッダー ──
	logo := headerStyle.Render("◆ RAG App")
...
func (m model) renderMessages() string {
	...
	for i, msg := range m.messages {
		...
		case RoleAssistant:
			ts := lipgloss.NewStyle().Foreground(colorMuted).Render(msg.Time.Format("15:04"))
			label := assistantLabelStyle.Render("◆ RAG App") + "  " + ts
			b.WriteString(label + "\n")
			// コードブロックの簡易レンダリング
...
func simulateAPICall(input string) tea.Cmd {
	return func() tea.Msg {
		time.Sleep(500 * time.Millisecond) // API レイテンシを模擬
        ...
        ragSearch, err := p.Lookup("RagSearch")
        if err != nil {
            fmt.Fprintf(os.Stderr, "❌ TodoAdd のルックアップに失敗: %v\n", err)
            os.Exit(1)
        }
        ragSearchString, ok := ragSearch.(func(string) string)
        if !ok {
            fmt.Fprintf(os.Stderr, "❌ TodoAdd の型が不正\n")
            os.Exit(1)
        }
        result := ragSearchString(inputLower)                      
        response = result	        					
